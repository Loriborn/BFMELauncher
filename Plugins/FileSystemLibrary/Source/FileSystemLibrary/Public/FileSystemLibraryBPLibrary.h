...
class UFileSystemLibraryBPLibrary : public UBlueprintFunctionLibrary
{
	...
	UFUNCTION(BlueprintCallable, meta = (DisplayName = "RunExecutable", Keywords = "FileSystemLibrary"), Category = "System File Operations")
	static FORCEINLINE bool RunExecutable(FString PathToFile, FString Directory, FString ExecParameters)
	{
		const TCHAR* Path = *PathToFile;
		const TCHAR* Param = *ExecParameters;
		const TCHAR* Working = *Directory;

		FPlatformProcess::CreateProc(Path, Param, true, false, false, nullptr, 0, Working, nullptr, nullptr);
